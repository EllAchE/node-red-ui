...
if (
  parsedArgs.safe ||
  (process.env.NODE_RED_ENABLE_SAFE_MODE &&
    !/^false$/i.test(process.env.NODE_RED_ENABLE_SAFE_MODE))
) {
...
if (process.env.NODE_RED_ENABLE_PROJECTS) {
  ...
  settings.editorTheme.projects.enabled = !/^false$/i.test(
    process.env.NODE_RED_ENABLE_PROJECTS
  );
}

if (process.env.NODE_RED_ENABLE_TOURS) {
  settings.editorTheme = settings.editorTheme || {};
  settings.editorTheme.tours = !/^false$/i.test(
    process.env.NODE_RED_ENABLE_TOURS
  );
}
  ...
  .then(function (startupHttps) {
    if (startupHttps) {
      ...
      if (settings.httpsRefreshInterval) {
        var httpsRefreshInterval =
          parseFloat(settings.httpsRefreshInterval) || 12;
        ...
        if (typeof settings.https === 'function') {
          ...
          setInterval(function () {
            try {
              ...
              Promise.resolve(settings.https())
                .then(function (refreshedHttps) {
                  if (refreshedHttps) {
                    ...
                    if (updateKey || updateCert) {
                      ...
                      RED.log.info(
                        RED.log._('server.https.settings-refreshed')
                      );
                    }
                  }
                })
                .catch(function (err) {
                  RED.log.error(
                    RED.log._('server.https.refresh-failed', { message: err })
                  );
                });
            } catch (err) {
              RED.log.error(
          ...
          delayedLogItems.push({
            type: 'warn',
            id: 'server.https.function-required',
          });
        }
      }
    } else {
    ...
    if (settings.httpStatic) {
      ...
      const statics = Array.isArray(settings.httpStatic)
        ? settings.httpStatic
        : [settings.httpStatic];
      const sanitised = [];
      for (let si = 0; si < statics.length; si++) {
        ...
        sp.root = formatRoot(
          path.posix.join(settings.httpStaticRoot, sp.subRoot)
        );
      }
    ...
    function basicAuthMiddleware(user, pass) {
      ...
      if (pass.length == '32') {
        ...
        checkPassword = function (p) {
          return (
            crypto.createHash('md5').update(p, 'utf8').digest('hex') === pass
          );
        };
      } else {
        checkPassword = function (p) {
    ...
    if (settings.httpNodeRoot !== false && settings.httpNodeAuth) {
      if (
        typeof settings.httpNodeAuth === 'function' ||
        Array.isArray(settings.httpNodeAuth)
      ) {
      ...
      .then(function () {
        if (
          ...
          delayedLogItems.forEach(function (delayedLogItem, index) {
            RED.log[delayedLogItem.type](
              RED.log._(delayedLogItem.id, delayedLogItem.params || {})
            );
          });
